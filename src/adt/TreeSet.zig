...
pub const Node = struct {
    ...
    /// Returns the next node or null if it is the last element.
    ...
    /// Returns the previous node or null if it is the first element.
...
pub fn searchMin(self: *Self, node: *Node) ?*Node {
    var nextOrNull = self.root;
...
pub fn searchMax(self: *Self, node: *Node) ?*Node {
    var nextOrNull = self.root;
    var result: ?*Node = null;
    while (nextOrNull) |next| {
        const order = self.compare(node, next);
...
pub fn insert(self: *Self, node: *Node) void {
    ...
    while (true) {
        order = self.compare(node, parent);
